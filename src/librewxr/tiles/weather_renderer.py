...
def colorize_weather_values(
    ...
    rgba = np.empty((*physical.shape, 4), dtype=np.uint8)
    finite = np.isfinite(physical)
    rgba[~finite] = palette.nodata_rgba

    below = finite & (physical < palette.minimum)
    above = finite & (physical > palette.maximum)
    inside = finite & ~below & ~above
    rgba[below] = palette.below_rgba
    rgba[above] = palette.above_rgba
    if inside.any():
        normalized = (
        ...
        rgba[inside] = palette.lut[indices]
    return rgba
